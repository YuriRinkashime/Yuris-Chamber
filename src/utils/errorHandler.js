...
//
// Rules:
// 1. Commands/handlers: throw YurisChamberError (via createError) or let errors propagate;
...
export class YurisChamberError extends Error {
    constructor(message, type = ErrorTypes.UNKNOWN, userMessage = null, context = {}) {
        super(message);
        this.name = 'YurisChamberError';
        this.type = type;
...
export function categorizeError(error) {
    if (error instanceof YurisChamberError) {
        return error.type;
    }
...
export function createError(message, type = ErrorTypes.UNKNOWN, userMessage = null, context = {}) {
    ...
    return new YurisChamberError(message, type, userMessage, normalizedContext);
}
