    ...
    return runWithTraceContext(interactionTraceContext, async () => {
      try {
        ...
        const isDmInboxBtn =
          ...
          (interaction.customId?.startsWith('dm_ai:') ||
            interaction.customId?.startsWith('dm_human:'));
        const isDmModal =
          interaction.isModalSubmit() &&
          interaction.customId?.startsWith('dm_modal:');

        if (
          (isMaintenanceMode() || isMaintenanceModeRuntime()) &&
        ...
        if (isDmInboxBtn) {
          const { handleDmOwnerButton } = await import(
            '../interactions/buttons/dm/dmReply.js'
          );
          return handleDmOwnerButton(interaction, client);
        }

        if (isDmModal) {
          const userId = interaction.customId.split(':')[1];
          const { default: modal } = await import(
            '../interactions/modals/dm_modal.js'
          );
          return modal.execute(interaction, client, [userId]);
        }

        if (
          interaction.isModalSubmit() &&
          interaction.customId?.startsWith('say_modal:')
        ) {
          const channelId = interaction.customId.split(':')[1];
          const { default: modal } = await import(
            '../interactions/modals/say_modal.js'
          );
          return modal.execute(interaction, client, [channelId]);
        }
        ...
        if (interaction.isChatInputCommand()) {
          try {
            ...
            // ===== Custom error: server commands in DMs =====
            if (!interaction.guildId) {
              ...
              if (!DM_ALLOWED.has(interaction.commandName)) {
                throw createError(
                  `Command ${interaction.commandName} used in DM`,
                  ErrorTypes.CONFIGURATION,
                  "❌ **Server only**\n\nThis command can't be used in DMs.\nOpen a server where **Yuri's Chamber** is online and run it there.",
                  withTraceContext(
                    {
                      commandName: interaction.commandName,
                      subtype: 'dm_blocked' },
            ...
            if (isMaintenanceMode() || isMaintenanceModeRuntime()) {
              throw createError(
          ...
          if (interaction.customId.startsWith('app_modal_')) {
            try {
              const { handleApplicationModal } = await import(
                '../utils/applicationModal.js'
              ).catch(() => ({}));
              if (handleApplicationModal) {
